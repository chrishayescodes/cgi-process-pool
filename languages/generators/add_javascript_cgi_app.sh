...
# Script to add a new JavaScript (Node.js) CGI application with full YARP integration  
# Usage: ./add_javascript_cgi_app.sh <app_name> <start_port> [instance_count]
...
echo "📜 Adding JavaScript (Node.js) CGI application: $APP_NAME"
echo "📡 Start port: $START_PORT"  
echo "🔢 Instances: $INSTANCE_COUNT"

# 1. Create JavaScript (Node.js) CGI application source
echo "📝 Creating JavaScript (Node.js) CGI source file..."
cat > "${APP_NAME}.js" << 'EOF'
# JavaScript (Node.js) Service Template for APP_NAME_PLACEHOLDER
# This would contain the actual service code template
# Generated by Language Manager for javascript
...
sed -i "s/APP_NAME_PLACEHOLDER/$APP_NAME/g" "${APP_NAME}.js"
...
if ! grep -q "${APP_NAME}" proxy/CGIProxy/Services/ProcessMonitorService.cs; then
    ...
    sed -i "s|cmdLine.Contains("${APP_NAME}.js") ? "${APP_NAME}" :|" proxy/CGIProxy/Services/ProcessMonitorService.cs
fi


echo ""
echo "✅ Successfully added $APP_NAME JavaScript (Node.js) service!"
echo ""
...
echo "   curl \"http://localhost:8080/api/$APP_NAME?health=true\""
echo ""
